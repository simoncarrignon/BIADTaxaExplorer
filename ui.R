...
ui <- fluidPage(
  tags$div(
    style = "background-color: #f8d7da; color: #721c24; padding: 10px; border-radius: 5px; margin-bottom: 20px; position: relative; z-index: 1000;",
    tags$h2("Warning: This is an experimental website for internal and testing use only.")
  ),
  tabsetPanel(
    tabPanel("Faunal Taxa", # Original tab for default dataset
      ...
      fluidRow(
        column(width = 6,
               selectInput("file_selector_tx", "Select File to group taxon:", 
                           choices = list.files(path = "groupings/faunal_taxa/", pattern = "group_.*.csv", full.names = TRUE),
                           selected = "groupings/faunal_taxa/group_IV.csv"),
               div(style = "font-size: 12px; margin-bottom: 10px;margin-left: 10px; margin-top: -10px;", 
    ...
    tabPanel("Botanical Taxa", 
      leafletOutput("map1", height = '60vh'),
      verbatimTextOutput("custom_groups"),
      actionButton("custom_run_btn", "Run Analysis (Custom)"),
      actionButton("custom_clear_polygons_btn", "Clear Polygons (Custom)"),
      downloadButton("custom_download_polygons", "Download Polygons (Custom)"),
      downloadButton("custom_download_table", "Download Data (Custom)"),
      fluidRow(
        column(width = 6,
               fileInput("custom_file_selector_tx", "Upload Custom Taxon File:", accept = c(".csv")),
               div(style = "font-size: 12px; margin-bottom: 10px;margin-left: 10px; margin-top: -10px;", 
                   htmlOutput("custom_taxon_table")  # Output for custom table
               )
        ),
      ),
      HTML("<p><b>Note:</b> Custom dataset functionalities can be explored here.</p>"),
      # Including similar UI components for plots and data if applicable
      fluidRow(
        column(width = 6, plotOutput("custom_plot1", height = "300px")),
        column(width = 6, plotOutput("custom_plot2", height = "300px"))
      ),
      fluidRow(
        column(width = 6, plotOutput("custom_plot3", height = "500px"))
      )
    )
  )
)
